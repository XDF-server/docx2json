...
class Dic2json(object):
    ...
    def _data_push(self,val):
        ...
        if gl.type_status == "options" :
            ...
            if (gl.q_type=="填空题" or gl.q_type == "词汇运用" or gl.q_type == "句型转换" or gl.q_type == "填空型完形填空" or gl.q_type == "填空型阅读理解") and gl.type_status=="body" and val_o['type']=='text':
                if val['style']==4 and re.match('^[ 　]+$'.decode('utf8'),val_o['value']):
                    if self.blank_flg==0:
                        self.blank_flg = 1
                        self.blank_cnt += 1
                        val={ "type" : "blank", "value" : self.blank_cnt }
                        gl.b_num=self.blank_cnt
    ...
    def _picinfo(self,val,docname):
        ...
        if x or xl or y or yl:
            print "change:" + str(width) + "x" + str(height) + " " + str(x) + "x" + str(y)
            #print x
            #print xl
            #print y
            #print yl
        ...
        if height_o and height/height_o/1.35 > 1.5 and width_o and width/width_o/1.35 > 1.5:
            #print "real:" + str(width_oe + "x" + str(height_o)
